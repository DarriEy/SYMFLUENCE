#!/usr/bin/env python3
"""
Bow at Banff Multivariate Evaluation - Publication Figure (v3)
Section 4.10a: GRACE TWS + Streamflow + SWE + Soil Moisture

Now includes:
- Observation station locations on map
- ISMN soil moisture comparison
"""
...
def load_summa_output(experiment_id='bow_tws_uncalibrated'):
    ...
    # Soil moisture by layer (layers 5,6,7 are soil at 0.2m, 0.3m, 0.5m)
    vwc = ds_day['mLayerVolFracLiq'].values  # (time, midToto, hru)
    data['SM_20cm'] = vwc[:, 5, 0]  # 0.2m layer
    data['SM_30cm'] = vwc[:, 6, 0]  # 0.3m layer
    data['SM_50cm'] = vwc[:, 7, 0]  # 0.5m layer
    # Average of all soil layers
    data['SM_mean'] = np.mean(vwc[:, 5:8, 0], axis=1)
...
def load_ismn_soil_moisture():
    """Load ISMN soil moisture observations."""
    ismn_dir = DATA_DIR / "observations/soil_moisture/ismn"

    if not ismn_dir.exists():
        return None, None

    # Load station metadata
    stations_file = ismn_dir / "ismn_station_selection.csv"
    df_stations = pd.read_csv(stations_file) if stations_file.exists() else None

    # Load all soil moisture files for 20cm depth (depth_147)
    all_data = []
    for f in sorted(ismn_dir.glob("*_depth_147.csv")):  # 0.2m depth
        df = pd.read_csv(f, parse_dates=['DateTime'])
        df['station_id'] = int(f.stem.split('_')[0])
        all_data.append(df)
    ...
    df_daily = df_all.groupby(df_all.index.date)['soil_moisture'].mean()
    df_daily = pd.DataFrame({'obs_SM': df_daily.values}, index=pd.to_datetime(df_daily.index))

    print(f"  ISMN soil moisture: {len(df_daily)} days at 0.2m depth")
    print(f"  SM range: {df_daily['obs_SM'].min():.3f} - {df_daily['obs_SM'].max():.3f} m³/m³")
...
def plot_domain_map_with_stations(gdf, df_swe_stations, df_sm_stations, ax):
    ...
    if df_swe_stations is not None:
        ...
        if len(stations_in) > 0:
            ax.scatter(stations_in['lon'], stations_in['lat'],
                       c=COLORS['station_swe'], s=35, marker='s', alpha=0.9,
                       edgecolors='#08519C', linewidth=1, zorder=9,
                       label=f'CanSWE in basin ({n_inside})')

        # Plot stations OUTSIDE catchment (hollow markers)
        if len(stations_out) > 0:
            ax.scatter(stations_out['lon'], stations_out['lat'],
                       facecolors='none', edgecolors=COLORS['station_swe'], s=30, marker='s',
                       alpha=0.7, linewidth=1, zorder=8,
                       label=f'CanSWE nearby ({n_outside})')

    # Pour point / WSC gauge
    ax.plot(BOW_LON, BOW_LAT, 'r^', markersize=10, markeredgecolor='white',
    ...
    # Legend
    ax.legend(loc='lower right', fontsize=6, framealpha=0.9, markerscale=0.8)
    ...
    ax.set_title('(a) Study Domain & Stations', fontweight='bold')
...
def plot_tws_comparison(df_sim, df_grace, metrics, ax):
    ...
    ax.plot(df_grace.index, df_grace['GRACE_TWS'].values, color=COLORS['grace'],
            linewidth=1.8, marker='o', markersize=3, alpha=0.9, label='GRACE')
...
def plot_swe(df_sim, df_swe, metrics, ax):
    ...
    ax.plot(obs_weekly.index, obs_weekly.values, color=COLORS['obs'],
            linewidth=0.8, alpha=0.8, label='CanSWE')
    ax.plot(sim_weekly.index, sim_weekly.values, color=COLORS['sim'],
            linewidth=0.8, alpha=0.8, label='Simulated')
...
def plot_soil_moisture(df_sim, df_sm, metrics, ax):
    """Plot soil moisture comparison."""
    if df_sm is None:
        ax.text(0.5, 0.5, 'ISMN soil moisture data not available',
                ha='center', va='center', transform=ax.transAxes)
        ax.set_title('(e) Soil Moisture (20cm)', fontweight='bold')
        return

    df_sim = filter_period(df_sim)
    df_sm = filter_period(df_sm)

    add_period_shading(ax)

    # Weekly means
    sim_weekly = df_sim['SM_20cm'].resample('W').mean()
    obs_weekly = df_sm['obs_SM'].resample('W').mean()

    ax.plot(obs_weekly.index, obs_weekly.values, color=COLORS['sm_obs'],
            linewidth=0.8, alpha=0.8, label='ISMN (20cm)')
    ax.plot(sim_weekly.index, sim_weekly.values, color=COLORS['sim'],
            linewidth=0.8, alpha=0.8, label='Simulated (20cm)')
    ...
    ax.set_title('(e) Soil Moisture (20cm depth)', fontweight='bold')
    ax.legend(loc='upper right', framealpha=0.9, fontsize=7)
    ax.xaxis.set_major_locator(mdates.YearLocator(2))
    ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y'))
    ax.set_xlim(pd.Timestamp(CALIBRATION_START), pd.Timestamp(EVALUATION_END))
...
def plot_scatter_grid(df_sim, df_grace, df_obs, df_swe, df_sm, axs):
    """Plot scatter comparison grid."""
    df_sim = filter_period(df_sim)
    ...
    if len(common) > 0:
        ax.scatter(grace_p.loc[common], sim_p.loc[common], c=COLORS['grace'], s=20, alpha=0.7)
    ...
    ax.set_title('(f) TWS', fontsize=9, fontweight='bold')
    ...
    if len(common) > 0:
        ax.scatter(df_q.loc[common, 'obs_Q'], sim_q.loc[common], c=COLORS['obs'], s=20, alpha=0.7)
    ...
    ax.set_title('(g) Streamflow', fontsize=9, fontweight='bold')
    ax.tick_params(labelsize=7)
    ...
    ax.set_ylabel('Simulated (mm)', fontsize=8)
    ax.set_title('(h) SWE', fontsize=9, fontweight='bold')
    ax.tick_params(labelsize=7)
    ...
    ax.plot(months, [sim_seasonal.get(m, np.nan) for m in months], 'o-',
            color=COLORS['sim'], linewidth=1.5, markersize=4, label='Sim')
    ...
    ax.set_title('(i) Seasonal Cycle', fontsize=9, fontweight='bold')
...
def main():
    ...
    print("=" * 60)
    print("Bow at Banff - Publication Figure (v3)")
    print("=" * 60)
    ...
    df_sm, df_sm_stations = load_ismn_soil_moisture()
    ...
    print(f"  Streamflow: r={q_metrics['r']:.3f}, KGE={q_metrics['KGE']:.3f}")
    ...
    print(f"  SWE: r={swe_metrics['r']:.3f}, Bias={swe_metrics['bias']:.1f} mm")

    # Soil moisture metrics
    if df_sm is not None:
        df_sm_filt = filter_period(df_sm)
        sm_metrics = calculate_metrics(df_sim_filt['SM_20cm'], df_sm_filt['obs_SM'], use_period=False)
        print(f"  Soil moisture: r={sm_metrics['r']:.3f}")
    else:
        sm_metrics = {'r': np.nan, 'RMSE': np.nan}

    # Create figure
    print("\nCreating figure...")
    fig = plt.figure(figsize=(7.5, 11))
    gs = GridSpec(5, 2, figure=fig, height_ratios=[1.0, 0.7, 0.7, 0.7, 0.9],
                  hspace=0.35, wspace=0.3)

    # (a) Domain map with stations
    ax_map = fig.add_subplot(gs[0, 0])
    plot_domain_map_with_stations(gdf, df_swe_stations, df_sm_stations, ax_map)
    ...
    # (e) Soil moisture
    ax_sm = fig.add_subplot(gs[3, :])
    plot_soil_moisture(df_summa, df_sm, sm_metrics, ax_sm)

    # Scatter panels
    gs_scatter = gs[4, :].subgridspec(1, 4, wspace=0.45)
    ax_scatter = [fig.add_subplot(gs_scatter[0, i]) for i in range(4)]
    plot_scatter_grid(df_summa, df_grace, df_obs, df_swe, df_sm, ax_scatter)
    ...
    # Save
    output_png = OUTPUT_DIR / "bow_banff_figure_v3.png"
    output_pdf = OUTPUT_DIR / "bow_banff_figure_v3.pdf"
    ...
    metrics_df = pd.DataFrame([
        ...
        {'Variable': 'Soil Moisture', 'Metric': 'r', 'Value': sm_metrics['r'], 'Period': '2004-2017'},
    ])
    metrics_df.to_csv(OUTPUT_DIR / "bow_banff_metrics_v3.csv", index=False)
