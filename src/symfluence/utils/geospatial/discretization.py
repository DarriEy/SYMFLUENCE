...
from symfluence.utils.geospatial.raster_utils import calculate_annual_radiation, calculate_aspect
...
class DomainDiscretizer:
    ...
    def __init__(self, config, logger):
        ...
        self.dem_path = self._get_file_path(
            "DEM_PATH", "attributes/elevation/dem", dem_name
        )
    ...
    def _discretize_combined(self, attributes: List[str]):
        ...
        if gru_shapefile == "default":
            gru_shapefile = self._get_file_path(
                "RIVER_BASINS_PATH",
        ...
        output_shapefile = self._get_file_path(
            "CATCHMENT_PATH",
            "shapefiles/catchment",
            f"{self.domain_name}_HRUs_{method_suffix}.shp",
        )
        output_plot = self._get_file_path(
            "CATCHMENT_PLOT_DIR",
            "plots/catchment",
    ...
    def _get_raster_info_for_attributes(
        ...
        for attr in attributes:
            ...
            if attr_lower == "elevation":
                ...
                raster_path = self._get_file_path(
                    "DEM_PATH", "attributes/elevation/dem", dem_name
                )
                ...
                radiation_raster = self._get_file_path(
                    "RADIATION_PATH", "attributes/radiation", "annual_radiation.tif"
                )
                ...
                if not radiation_raster.exists():
                    ...
                    if dem_name == "default":
                        dem_name = f"domain_{self.config.get('DOMAIN_NAME')}_elv.tif"
                    dem_raster = self._get_file_path(
                        "DEM_PATH", "attributes/elevation/dem", dem_name
                    )
                    radiation_raster = calculate_annual_radiation(
                        dem_raster, radiation_raster, self.logger
                    )
                ...
                aspect_raster = self._get_file_path(
                    "ASPECT_PATH", "attributes/aspect", "aspect.tif"
                )
                ...
                if not aspect_raster.exists():
                    ...
                    aspect_raster = calculate_aspect(
                        dem_raster,
                        aspect_raster,
                        int(self.config.get("ASPECT_CLASS_NUMBER", 8)),
                        self.logger,
                    )
    ...
    def _classify_continuous_data(
        self, raster_array: np.ndarray, valid_mask: np.ndarray, band_size: float
    ) -> np.ndarray:
        """
        ...
        if (
            isinstance(band_size, int) and band_size < 50
        ):  # Assume it's number of classes for radiation
    ...
    def _find_unique_combinations(
        self, classified_data: Dict[str, np.ndarray], valid_mask: np.ndarray
    ) -> List[Tuple]:
        """
    ...
    def _use_grus_as_hrus(self):
        ...
        if self.config.get("RIVER_BASINS_NAME") == "default":
            gru_shapefile = self._get_file_path(
                "RIVER_BASINS_PATH",
                "shapefiles/river_basins",
                f"{self.domain_name}_riverBasins_{self.config.get('DOMAIN_DEFINITION_METHOD')}.shp",
            )
            ...
            if self.config.get("DELINEATE_COASTAL_WATERSHEDS") == True:
                ...
                gru_shapefile = self._get_file_path(
                    "RIVER_BASINS_PATH",
                    "shapefiles/river_basins",
                    f"{self.domain_name}_riverBasins_point.shp",
                )
        ...
        hru_output_shapefile = self._get_file_path(
            "CATCHMENT_PATH",
            "shapefiles/catchment",
        ...
        try:
            ...
            self.logger.info(
                f"Calculated centroids in WGS84: lat range {centroids_wgs84.y.min():.6f} to {centroids_wgs84.y.max():.6f}, lon range {centroids_wgs84.x.min():.6f} to {centroids_wgs84.x.max():.6f}"
            )
    ...
    def _discretize_by_elevation(self):
        ...
        if gru_shapefile == "default":
            ...
            gru_shapefile = self._get_file_path(
                "RIVER_BASINS_PATH",
                "shapefiles/river_basins",
                f"{self.domain_name}_riverBasins__with_coastal.shp",
            )
        else:
            gru_shapefile = self._get_file_path(
                "RIVER_BASINS_PATH",
        ...
        output_shapefile = self._get_file_path(
            "CATCHMENT_PATH",
            "shapefiles/catchment",
            f"{self.domain_name}_HRUs_elevation.shp",
        )
        output_plot = self._get_file_path(
            "CATCHMENT_PLOT_DIR",
            "plots/catchment",
            f"{self.domain_name}_HRUs_elevation.png",
        )

        elevation_band_size = float(self.config.get("ELEVATION_BAND_SIZE"))
        gru_gdf, elevation_thresholds = self._read_and_prepare_data(
            gru_shapefile, dem_raster, elevation_band_size
        )
        hru_gdf = self._create_multipolygon_hrus(
    ...
    def _discretize_by_soil_class(self):
        ...
        soil_raster = self._get_file_path(
            "SOIL_CLASS_PATH",
            "attributes/soilclass/",
            f"domain_{self.config.get('DOMAIN_NAME')}_soil_classes.tif",
        )
        output_shapefile = self._get_file_path(
            "CATCHMENT_PATH",
            "shapefiles/catchment",
            f"{self.domain_name}_HRUs_soilclass.shp",
        )
        output_plot = self._get_file_path(
            "CATCHMENT_PLOT_DIR",
            "plots/catchment",
            f"{self.domain_name}_HRUs_soilclass.png",
        )
    ...
    def _discretize_by_land_class(self):
        ...
        land_raster = self._get_file_path(
            "LAND_CLASS_PATH",
            "attributes/landclass",
            f"domain_{self.config.get('DOMAIN_NAME')}_land_classes.tif",
        )
        output_shapefile = self._get_file_path(
            "CATCHMENT_PATH",
            "shapefiles/catchment",
            f"{self.domain_name}_HRUs_landclass.shp",
        )
        output_plot = self._get_file_path(
            "CATCHMENT_PLOT_DIR",
            "plots/catchment",
            f"{self.domain_name}_HRUs_landclass.png",
        )
    ...
    def _discretize_by_aspect(self):
        ...
        if not aspect_raster.exists():
            self.logger.info("Aspect raster not found. Calculating aspect...")
            aspect_raster = calculate_aspect(
                dem_raster, aspect_raster, aspect_class_number, self.logger
            )
            if aspect_raster is None:
                raise ValueError("Failed to calculate aspect")

        gru_gdf, aspect_classes = self._read_and_prepare_data(
    ...
    def _discretize_by_radiation(self):
        ...
        output_shapefile = self._get_file_path(
            "CATCHMENT_PATH",
            "shapefiles/catchment",
            f"{self.domain_name}_HRUs_radiation.shp",
        )
        output_plot = self._get_file_path(
            "CATCHMENT_PLOT_DIR",
            "plots/catchment",
            f"{self.domain_name}_HRUs_radiation.png",
        )
        ...
        gru_gdf, radiation_thresholds = self._read_and_prepare_data(
