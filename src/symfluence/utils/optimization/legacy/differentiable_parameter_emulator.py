...
.. deprecated::
    ...
    Consider using the gradient-based optimization methods (run_adam, run_lbfgs)
    available in the new unified model optimizers:

    - SUMMAModelOptimizer.run_adam()
    - SUMMAModelOptimizer.run_lbfgs()
    - FUSEModelOptimizer.run_adam()
    - FUSEModelOptimizer.run_lbfgs()
    - NgenModelOptimizer.run_adam()
    - NgenModelOptimizer.run_lbfgs()

    To use the new infrastructure, set USE_UNIFIED_OPTIMIZER=True in your config
    and use ITERATIVE_OPTIMIZATION_ALGORITHM='ADAM' or 'LBFGS'.
...
class DifferentiableParameterOptimizer:
    ...
    def __init__(self, symfluence_config: Dict, domain_name: str, emulator_config: EmulatorConfig = None):
        import warnings
        warnings.warn(
            "DifferentiableParameterOptimizer is deprecated and will be removed in a future version. "
            "Use the gradient-based methods (run_adam, run_lbfgs) in model-specific optimizers instead. "
            "Set USE_UNIFIED_OPTIMIZER=True in your config.",
            DeprecationWarning,
            stacklevel=2
        )
