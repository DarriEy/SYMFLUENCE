...
class MizuRoutePreProcessor(BaseModelPreProcessor):
    ...
    def run_preprocessing(self):
        ...
        if self.typed_config:
            needs_remap = self.typed_config.model.mizuroute.needs_remap if self.typed_config.model.mizuroute else False
            from_model = self.typed_config.model.mizuroute.from_model if self.typed_config.model.mizuroute else None
            fuse_routing = self.typed_config.model.fuse.routing_integration if self.typed_config.model.fuse else None
            gr_routing = self.typed_config.model.gr.routing_integration if self.typed_config.model.gr else None
        else:
            needs_remap = self.config.get('SETTINGS_MIZU_NEEDS_REMAP')
            from_model = self.config.get('MIZU_FROM_MODEL')
            fuse_routing = self.config.get('FUSE_ROUTING_INTEGRATION')
            gr_routing = self.config.get('GR_ROUTING_INTEGRATION')
    ...
    def _write_gr_control_file_runoff(self, cf):
        ...
        cf.write("!\n! --- DEFINE RUNOFF FILE \n")
        cf.write(f"<fname_qsim>            {self.config.get('EXPERIMENT_ID')}_timestep.nc    ! netCDF name for GR4J runoff \n")
