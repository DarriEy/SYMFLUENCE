...
class OutputConverterMixin:
    ...
    def convert_fuse_to_mizuroute(self, output_file: Path) -> Path:
        """
        Convert FUSE output to mizuRoute format.

        Convenience method with FUSE-specific defaults:
        - Squeezes singleton longitude dimension
        - Renames latitude → gru
        - Adds gruId variable

        Args:
            output_file: Path to FUSE output netCDF file

        Returns:
            Path to converted file
        """
        return self.convert_to_mizuroute_format(
    ...
    def convert_gr_to_mizuroute(self, output_file: Path) -> Path:
        """
        Convert GR output to mizuRoute format.

        Convenience method with GR-specific defaults.

        Args:
            output_file: Path to GR output netCDF file
