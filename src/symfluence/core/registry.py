...
def model_manifest(
    ...
    *,
    ...
    # ------------------------------------------------------------------
    # Backward compatibility: also populate legacy registries so that
    # existing consumers (which still read from old registries) continue
    # to work.  These writes will be removed in Phase 4.
    # ------------------------------------------------------------------
    _populate_legacy_registries(
        model_name,
        ...
        config_transformers=config_transformers,
        ...
        koopman_analyzer=koopman_analyzer,
        plotter=plotter,
        forcing_adapter=forcing_adapter,
    )


def _populate_legacy_registries(
    model_name: str,
    **kwargs: Any,
) -> None:
    """Write to old registries for backward compatibility during transition.

    Each write is wrapped in a try/except so a missing old registry never
    blocks model loading.  Will be removed in Phase 4.
    """
    ...
    try:
        ...
        if kwargs.get("preprocessor") is not None:
            ComponentRegistry._preprocessors[key] = kwargs["preprocessor"]
        if kwargs.get("runner") is not None:
            ComponentRegistry._runners[key] = kwargs["runner"]
            ComponentRegistry._runner_methods[key] = kwargs.get("runner_method") or "run"
        if kwargs.get("postprocessor") is not None:
            ComponentRegistry._postprocessors[key] = kwargs["postprocessor"]
        if kwargs.get("visualizer") is not None:
            ComponentRegistry._visualizers[key] = kwargs["visualizer"]
    except ImportError:
        pass
    ...
    try:
        ...
        if kwargs.get("config_adapter") is not None:
            ConfigRegistry._config_adapters[key] = kwargs["config_adapter"]
        if kwargs.get("config_schema") is not None:
            ConfigRegistry._config_schemas[key] = kwargs["config_schema"]
        if kwargs.get("config_defaults") is not None:
            ConfigRegistry._config_defaults[key] = kwargs["config_defaults"]
        if kwargs.get("config_transformers") is not None:
            ConfigRegistry._config_transformers[key] = kwargs["config_transformers"]
        if kwargs.get("config_validator") is not None:
            ConfigRegistry._config_validators[key] = kwargs["config_validator"]
    except ImportError:
        pass

    # Phase 4: ResultExtractorRegistry, AnalysisRegistry, PlotterRegistry,
    # and ForcingAdapterRegistry are now pure delegation shims (state lives
    # in R.*).  No legacy back-writes needed for those registries.

    try:
        from symfluence.optimization.registry import OptimizerRegistry

        if kwargs.get("optimizer") is not None:
            OptimizerRegistry._optimizers[key] = kwargs["optimizer"]
        if kwargs.get("worker") is not None:
            OptimizerRegistry._workers[key] = kwargs["worker"]
        if kwargs.get("parameter_manager") is not None:
            OptimizerRegistry._parameter_managers[key] = kwargs["parameter_manager"]
    except ImportError:
        pass
