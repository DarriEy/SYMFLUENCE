...
class SpatialOrchestrator(ABC):
    ...
    def _run_mizuroute(
        ...
        try:
            ...
            if runner_cls is None:
                self.logger.warning("mizuRoute runner not registered - skipping routing")
                return None
    ...
    def _create_mizuroute_control_file(self, model_name: str) -> None:
        ...
        try:
            ...
            preprocessor_cls = R.preprocessors.get('MIZUROUTE')
            if preprocessor_cls is None:
                self.logger.warning(
                    "mizuRoute preprocessor not registered - skipping control file creation")
                return
    ...
    def _run_troute(
        ...
        try:
            ...
            if runner_cls is None:
                self.logger.warning("t-route runner not registered - skipping routing")
                return None
