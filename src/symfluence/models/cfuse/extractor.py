...
Provides utilities for extracting and analyzing cFUSE model results
...
class CFUSEResultExtractor:
    """
    Utility class for extracting cFUSE simulation results.
    ...
    def __init__(
        ...
        Args:
            output_dir: Directory containing cFUSE output files
    ...
    def extract_distributed_runoff(self) -> Optional[xr.Dataset]:
        ...
        patterns = [
            ...
            "*_runs_def.nc",
            f"{self.domain_name}_cfuse_output_distributed.nc"
        ]
    ...
    def extract_states(self) -> Optional[pd.DataFrame]:
        ...
        """
        states_file = self.output_dir / f"{self.domain_name}_cfuse_states.nc"
        ...
        states = {}
        for var in ['S1', 'S2', 'snow', 'soil_moisture', 'upper_storage', 'lower_storage']:
            if var in ds:
    ...
    def compare_runs(
        ...
        """
        Compare results with another cFUSE run.
        ...
        """
        other_extractor = CFUSEResultExtractor(
            other_dir,
    ...
    def compare_model_structures(
        ...
        """
        Compare results from different cFUSE model structures.
        ...
        for structure_name, output_dir in structure_dirs.items():
            extractor = CFUSEResultExtractor(output_dir, self.domain_name, self.logger)
