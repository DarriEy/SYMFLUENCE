...
class RHESSysPreprocessor(BaseModelPreProcessor, ObservationLoaderMixin):
    ...
    def __init__(self, config, logger_instance):
        ...
        self.experiment_id = self._resolve_config_value(
            lambda: self.config.domain.experiment_id,
            'EXPERIMENT_ID',
            'default'
        )

        # Get forcing dataset info (keep uppercase for VariableHandler compatibility)
    ...
    def _generate_climate_files(self):
        ...
        Required variables: precipitation (rain), temperature (tmax, tmin, tavg)
        """
        logger.info("Generating climate files...")

        try:
            ds = self._load_forcing_data()
        except FileNotFoundError as e:
            logger.warning(f"Could not load forcing data: {e}")
            logger.info("Creating synthetic climate data for testing")
            self._create_synthetic_climate()
            return
        ...
        var_handler = VariableHandler(self.config_dict, self.logger, self.forcing_dataset, 'RHESSys')
        ...
        # Helper function to basin-average multi-dimensional data
        def basin_average(data_array):
            """Average across all spatial dimensions (HRU, GRU, etc.)"""
            # Get the data values
            values = data_array.values
            # If more than 1D, average across all non-time dimensions
            if values.ndim > 1:
                # Time is typically first dimension; average all others
                spatial_axes = tuple(range(1, values.ndim))
                values = np.nanmean(values, axis=spatial_axes)
            return values.flatten()

        # Get precipitation (convert to mm/hour for later daily summing)
        if precip_var:
            precip = basin_average(ds[precip_var])
            # Convert from kg/m2/s to mm/hour for summing to daily
            # kg/m2/s = mm/s, so multiply by 3600 to get mm/hour
            # Then daily sum will give mm/day
            if 'kg' in str(ds[precip_var].attrs.get('units', '')).lower() or 's-1' in str(ds[precip_var].attrs.get('units', '')):
                precip = precip * 3600  # kg/m2/s to mm/hour (will be summed to mm/day)
        else:
            logger.warning("No precipitation variable found, using zeros")
            precip = np.zeros(len(dates))

        # Get temperature (convert to Celsius if needed)
        if temp_var:
            temp = basin_average(ds[temp_var])
            # Convert from Kelvin if needed
            if np.nanmean(temp) > 200:  # Likely Kelvin
                temp = temp - 273.15
        else:
            logger.warning("No temperature variable found, using 10C default")
            temp = np.full(len(dates), 10.0)

        # Get or estimate tmax/tmin
        if tmax_var:
            tmax = basin_average(ds[tmax_var])
            if np.nanmean(tmax) > 200:
                tmax = tmax - 273.15
        else:
            tmax = temp + 5  # Estimate

        if tmin_var:
            tmin = basin_average(ds[tmin_var])
            if np.nanmean(tmin) > 200:
                tmin = tmin - 273.15
        else:
            tmin = temp - 5  # Estimate

        # Get shortwave radiation (W/m2)
        if swrad_var:
            swrad = basin_average(ds[swrad_var])
            logger.info(f"Loaded shortwave radiation from {swrad_var}")
        else:
            # Estimate from latitude and day of year (rough approximation)
            logger.warning("No shortwave radiation variable found, will estimate from lat/day")
            swrad = None

        # Get longwave radiation (W/m2)
        if lwrad_var:
            lwrad = basin_average(ds[lwrad_var])
            logger.info(f"Loaded longwave radiation from {lwrad_var}")
        else:
            lwrad = None

        # Get wind speed (m/s)
        if wind_var:
            wind = basin_average(ds[wind_var])
            logger.info(f"Loaded wind speed from {wind_var}")
        else:
            wind = np.full(len(dates), 2.0)  # Default 2 m/s
            logger.warning("No wind variable found, using default 2 m/s")

        # Calculate relative humidity from specific humidity, pressure, and temperature
        if spechum_var and airpres_var:
            spechum = basin_average(ds[spechum_var])
            airpres = basin_average(ds[airpres_var])
            # Convert specific humidity to relative humidity
            # q = 0.622 * e / (p - 0.378 * e), where e = vapor pressure
            # Saturation vapor pressure: es = 611.2 * exp(17.67 * T / (T + 243.5)) [Pa]
            # At temperature T (Celsius)
            temp_for_rh = temp.copy()
            es = 611.2 * np.exp(17.67 * temp_for_rh / (temp_for_rh + 243.5))  # Saturation VP in Pa
            # From specific humidity: e = q * p / (0.622 + 0.378 * q)
            e = spechum * airpres / (0.622 + 0.378 * spechum)  # Actual VP in Pa
            rh = 100.0 * e / es  # Relative humidity in %
            rh = np.clip(rh, 0, 100)  # Ensure 0-100% range
            logger.info(f"Calculated relative humidity from {spechum_var} and {airpres_var}")
        else:
            rh = np.full(len(dates), 70.0)  # Default 70% RH
            logger.warning("Missing humidity/pressure variables, using default 70% RH")

        ds.close()

        # RHESSys uses DAILY data - aggregate hourly forcing to daily
        ...
        # Add radiation variables if available
        if swrad is not None:
            df_forcing['swrad'] = swrad
        if lwrad is not None:
            df_forcing['lwrad'] = lwrad

        # Resample to daily with appropriate aggregations
        agg_dict = {
            'precip': 'sum',    # Total daily precipitation (mm/day)
            'temp': 'mean',     # Mean daily temperature
            'tmax': 'max',      # Daily maximum temperature
            'tmin': 'min',      # Daily minimum temperature
            'wind': 'mean',     # Mean daily wind speed
            'rh': 'mean'        # Mean daily relative humidity
        }
        if swrad is not None:
            agg_dict['swrad'] = 'mean'  # Mean daily shortwave radiation
        if lwrad is not None:
            agg_dict['lwrad'] = 'mean'  # Mean daily longwave radiation

        df_daily = df_forcing.resample('D').agg(agg_dict)

        # Extract daily arrays
        daily_dates = df_daily.index
        precip_daily = df_daily['precip'].values
        temp_daily = df_daily['temp'].values
        tmax_daily = df_daily['tmax'].values
        tmin_daily = df_daily['tmin'].values
        wind_daily = df_daily['wind'].values
        rh_daily = df_daily['rh'].values

        # Extract optional radiation
        swrad_daily = df_daily['swrad'].values if swrad is not None else None
        lwrad_daily = df_daily['lwrad'].values if lwrad is not None else None

        logger.info(f"Aggregated {len(dates)} hourly values to {len(daily_dates)} daily values")

        # Create climate station base file
        base_station_id = 1
        base_name = f"{self.domain_name}_base"

        # Write base station file
        self._write_base_station_file(base_name, base_station_id, daily_dates[0])

        # Write individual climate files (daily values)
        # RHESSys expects precipitation in meters/day, not mm/day
        precip_m_day = precip_daily / 1000.0
        self._write_climate_file(f"{base_name}.rain", daily_dates, precip_m_day)
        self._write_climate_file(f"{base_name}.tmax", daily_dates, tmax_daily)
        self._write_climate_file(f"{base_name}.tmin", daily_dates, tmin_daily)
        self._write_climate_file(f"{base_name}.tavg", daily_dates, temp_daily)

        # Write additional climate files for ET calculation
        self._write_climate_file(f"{base_name}.wind", daily_dates, wind_daily)
        self._write_climate_file(f"{base_name}.relative_humidity", daily_dates, rh_daily)

        # Write radiation files if available
        if swrad_daily is not None:
            self._write_climate_file(f"{base_name}.Kdown_direct", daily_dates, swrad_daily)
            logger.info(f"Wrote shortwave radiation file")
        if lwrad_daily is not None:
            self._write_climate_file(f"{base_name}.Ldown", daily_dates, lwrad_daily)
            logger.info(f"Wrote longwave radiation file")

        logger.info(f"Climate files written to {self.climate_dir}")
