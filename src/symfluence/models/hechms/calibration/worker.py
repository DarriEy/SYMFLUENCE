"""
HEC-HMS Calibration Worker.

Worker implementation for HEC-HMS model optimization.
Runs HEC-HMS in-memory for efficient DDS/evolutionary calibration.
"""
...
@OptimizerRegistry.register_worker('HECHMS')
class HecHmsWorker(InMemoryModelWorker):
    ...
    def _get_forcing_variable_map(self) -> Dict[str, str]:
        """Return mapping from standard names to HEC-HMS variable names."""
        return {
            'precip': 'pr',
            'temp': 'temp',
            'pet': 'pet',
    ...
    def _run_simulation(
        ...
        **kwargs
        ...
        if not self._ensure_simulate_fn():
            raise RuntimeError("HEC-HMS simulation function not available")
    ...
    def _ensure_simulate_fn(self) -> bool:
        ...
        try:
            from symfluence.models.hechms.model import HAS_JAX as MODEL_HAS_JAX
            from symfluence.models.hechms.model import simulate
            self._simulate_fn = simulate
            self._use_jax = MODEL_HAS_JAX and HAS_JAX
            return True
        except ImportError as e:
            self.logger.error(f"Failed to import HEC-HMS model: {e}")
            return False
    ...
    def evaluate_worker_function(task_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return _evaluate_hechms_parameters_worker(task_data)


def _evaluate_hechms_parameters_worker(task_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        worker = HecHmsWorker(config=task_data.get('config'))
        ...
        return {
            ...
            'error': f'HEC-HMS worker exception: {str(e)}\n{traceback.format_exc()}',
