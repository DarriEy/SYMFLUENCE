"""
HydroGeoSphere Coupling Diagnostics & Visualization

Generates a multi-panel overview figure showing SUMMA+HGS coupling:
flow separation hydrograph, groundwater head, and flow duration curves.
"""
...
@PlotterRegistry.register_plotter('HYDROGEOSPHERE')
class HGSPlotter(BasePlotter):
    """Plotter for HGS coupling diagnostics."""
    ...
    def plot_coupling_results(self, experiment_id: str) -> Optional[str]:
        ...
        try:
            ...
            self.logger.warning(f"Could not collect HGS coupling data: {e}")
            return None
        ...
        if 'head' in data and len(data['head']) > 0:
            ax_head.plot(data['head'].index, data['head'].values,
                         color='#1f77b4', linewidth=1.2, label='PM Head')
            self._apply_standard_styling(
                ax_head, ylabel='Head (m)', title='Porous Media Head',
                legend_loc='lower left',
            )
            self._format_date_axis(ax_head, format_type='month', rotation=30)
        else:
            ax_head.text(0.5, 0.5, 'No head data', transform=ax_head.transAxes,
                         ha='center', va='center')
            ax_head.set_title('Porous Media Head')
        ...
        fig.suptitle(
            f'HydroGeoSphere Coupling Overview — {experiment_id}',
            fontsize=14, fontweight='bold', y=0.98,
        )

        output_dir = self._ensure_output_dir('hgs_coupling')
    ...
    def _collect_coupling_data(self, experiment_id: str) -> Dict[str, Any]:
        ...
        from symfluence.models.hydrogeosphere.coupling import SUMMAToHGSCoupler

        sim_base = self.project_dir / 'simulations' / experiment_id
        hgs_dir = sim_base / 'HYDROGEOSPHERE'
        ...
        hydrograph = extractor.extract_variable(
            hgs_dir, 'hydrograph', start_date=start_date,
        )
        baseflow_m3s = hydrograph

        try:
            head = extractor.extract_variable(
                hgs_dir, 'head', start_date=start_date,
            )
        except Exception:
