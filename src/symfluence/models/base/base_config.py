...
class AutoGeneratedConfigAdapter(ModelConfigAdapter):
    ...
    def get_field_transformers(self) -> Dict[str, Tuple[str, ...]]:
        ...
        # Fallback for adapters whose schema uses bare field names (no uppercase
        # Field(alias=...)) but declare a ``CONFIG_PREFIX`` — e.g. the JAX plugin
        # models (jHBV/jSACSMA/...). Derive the flat key as ``PREFIX + FIELD``.
        # Without this their keys are never registered, so core neither
        # recognizes them nor populates ``config.model.<model>`` (the model then
        # silently runs on schema defaults instead of the user's settings).
        if not mapping:
            prefix = getattr(self, 'CONFIG_PREFIX', None)
            if prefix and hasattr(schema, 'model_fields'):
                for field_name in schema.model_fields:
                    mapping[f'{prefix}{field_name.upper()}'] = ('model', model_name_lower, field_name)
