...
class AutoGeneratedConfigAdapter(ModelConfigAdapter):
    ...
    def get_field_transformers(self) -> Dict[str, Tuple[str, ...]]:
        ...
        # Generate mapping for this model's schema
        full_mapping = generate_flat_to_nested_map(schema, include_model_overrides=False)

        # Filter to this model's keys only
        model_prefix = self.model_name.upper()
        settings_prefix = f'SETTINGS_{model_prefix}'

        return {
            k: v for k, v in full_mapping.items()
            if k.startswith(model_prefix) or k.startswith(settings_prefix)
        }
