...
- **Allowlist completion — status.** A scope+fix pass reduced the distinct
  ...
  - **Done (T4):** verified-dead keys (read nowhere in core or plugins — e.g.
    `DPE_*`, `GAP_FILLING_*`, profiling) stripped from the four shipped flat
    templates.
  - **Done (plugin path, core-side):** the JAX plugin models
    (jHBV/jSACSMA/jHECHMS/jTOPMODEL, + xinanjiang/snow17) register only a
    `config_adapter` whose schema uses bare field names + a `CONFIG_PREFIX`.
    Two core fixes make their keys recognized **and** typed without any plugin
    edits: `AutoGeneratedConfigAdapter.get_field_transformers()` falls back to
    `CONFIG_PREFIX`+field when no aliases exist, and `model_manifest()` bridges
    an adapter's `get_config_schema()` into `R.config_schemas`. This also fixed
    a **latent bug** — those models were silently running on schema defaults
    because `config.model.<model>` was never built. (See the typed-config plugin
    path, ADR-0002.) jFUSE lacks a `CONFIG_PREFIX` and is handled separately.
  - **Enabling strict** therefore remains gated only on trimming any remaining
    dead/aspirational template keys — *then* CI and shipped examples can
    run strict.
