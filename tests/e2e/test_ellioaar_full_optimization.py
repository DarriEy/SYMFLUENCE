"""
End-to-end test for Elliðaár Iceland lumped SUMMA model with full optimization.

This comprehensive test validates the complete SYMFLUENCE workflow including:
- Cloud data acquisition (Copernicus DEM, MODIS, CARRA, WSC streamflow)
...
@pytest.fixture
def ellioaar_config():
    """Load Elliðaár Iceland SUMMA optimization config."""
    config_path = Path(__file__).parent.parent / "configs" / "test_ellioaar_summa_optimization.yaml"
    assert config_path.exists(), f"Config file not found: {config_path}"
    return config_path


def test_ellioaar_iceland_full_workflow(ellioaar_config, clear_cache_flag):
    """
    Full E2E test for Elliðaár (Iceland) lumped SUMMA model with optimization.

    This test performs a comprehensive workflow:
    1. Delineates lumped basin for Elliðaár (Reykjavik, Iceland)
    2. Acquires cloud data:
       - Copernicus DEM
       - MODIS landcover
       - CARRA forcings (2015-2019, 5 years)
       - WSC streamflow
    ...
    with open(ellioaar_config, 'r') as f:
    ...
    sym = SYMFLUENCE(ellioaar_config, visualize=True)
    ...
    if dem_files and not clear_cache_flag:
        ...
        print("Downloading Copernicus DEM for Elliðaár basin...")
    ...
    if forcing_files and not clear_cache_flag:
        print(f"✓ Using cached CARRA forcing data ({len(forcing_files)} files)")
    else:
        print("Acquiring forcing data (CARRA, 2015-2019)...")
        ...
        print(f"✓ CARRA forcing data acquired ({len(forcing_files)} files)")
    ...
    print("TEST SUMMARY")
    print("="*80)
    print(f"Basin: Elliðaár at Reykjavik, Iceland (WSC-05BB001)")
    print(f"Period: 2015-2019 (5 years)")
    print(f"Model: SUMMA (lumped)")
    print(f"Forcing: CARRA (Arctic Regional Reanalysis)")
