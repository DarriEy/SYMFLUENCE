...
"""CLM-ParFlow forcing ingestion resamples non-hourly source to hourly cadence.

Mirrors the ParFlow case: the rainfall pipeline (and the CLM met-forcing writer
it feeds) equate one forcing step with one hour, so a 3-hourly source must be
resampled to hourly at ingestion.
"""
...
def test_three_hourly_source_is_resampled_to_hourly(tmp_path):
    pp = CLMParFlowPreProcessor(_config(tmp_path), Mock())
    _write_forcing(pp, "3h", n=40)
