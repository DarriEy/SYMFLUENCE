...
class TestAutoGeneratedMapping:
    ...
    def test_cached_access_performance(self):
        ...
        # perf_counter, not time.time: on Windows time.time() advances in
        # ~15.6ms system-clock ticks, so a sub-tick generation measures as
        # exactly 0.0. That made both sides of the ratio below zero and failed
        # the assert on the Windows leg (`assert 0.0 < (0.0 / 10)`) while the
        # cache was working perfectly. perf_counter is sub-microsecond on every
        # supported platform.
        t._AUTO_GENERATED_MAP = None
        ...
        for _ in range(1000):
            mapping = get_flat_to_nested_map()
        warm_avg = (time.perf_counter() - start) / 1000

        assert len(mapping) > 400
        # Guard the ratio against an unmeasurably fast cold build: if the clock
        # cannot resolve it, the comparison carries no information and would
        # only reintroduce the flake on a coarser timer.
        if cold <= time.get_clock_info('perf_counter').resolution:
            pytest.skip(
                f"Cold generation ({cold * 1e6:.3f}us) is at or below timer "
                "resolution; ratio would be meaningless"
            )
